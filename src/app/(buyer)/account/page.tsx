      ...
      <div style={{ padding: "24px 28px", overflowY: "auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, marginBottom: 20 }}>
          {[
            ...
            { label: "Profile", emoji: "👤", href: "/account/profile" },
          ].map((q) => (
            <Link key={q.href} href={q.href} style={{ textDecoration: "none" }}>
              <div style={{ background: "white", border: "1px solid var(--border)", borderRadius: "var(--radius-md)", padding: "14px", textAlign: "center", cursor: "pointer" }}>
                <div style={{ fontSize: 22, marginBottom: 6 }}>{q.emoji}</div>
                <div style={{ fontSize: 12, fontWeight: 500, color: "var(--text-dark)" }}>{q.label}</div>
              </div>
            </Link>
          ))}
        </div>
